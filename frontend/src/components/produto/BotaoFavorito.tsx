...
export default function BotaoFavorito({ produtoId, size = 'sm' }: Props) {
  ...
  return (
    <button onClick={toggle} className={`${sz} rounded-full flex items-center justify-center transition-colors ${favoritado ? 'bg-red-50 text-red-500' : 'bg-white/80 text-gray-400 hover:text-red-400'}`} title={favoritado ? 'Remover dos favoritos' : 'Adicionar aos favoritos'}>
