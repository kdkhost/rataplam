  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        ...
        {filtros.busca && <p className="text-sm text-blue-600 mt-1">Resultados para: &quot;{filtros.busca}&quot; <button onClick={() => handleFiltro('busca', '')} className="text-red-500 hover:underline ml-2">(limpar)</button></p>}
      </div>
                ...
                <div className="flex justify-center gap-2 mt-8">
                  {Array.from({ length: totalPaginas }, (_, i) => i + 1).map((p) => (
                    <button key={p} onClick={() => { setPagina(p); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
                      className={`w-10 h-10 rounded-xl text-sm font-medium transition-colors ${p === pagina ? 'bg-blue-600 text-white' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'}`}>
                      {p}
                    </button>
                  ))}
                </div>
...
export default function LojaPage() {
  return (
    <Suspense fallback={<div className="max-w-7xl mx-auto px-4 py-20 text-center"><div className="animate-spin w-8 h-8 border-4 border-blue-600 border-t-transparent rounded-full mx-auto" /></div>}>
      <LojaConteudo />
    </Suspense>
  );
}
